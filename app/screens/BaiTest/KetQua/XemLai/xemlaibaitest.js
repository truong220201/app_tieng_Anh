...
import { Dimensions,TouchableOpacity,ScrollView, Text, View,Button,StyleSheet,Alert,BackHandler,TouchableWithoutFeedback,SafeAreaView,LinearGradient,Image} from 'react-native';
...
export default class xemlai extends React.Component{ 
    ...
    constructor(props) {
        ...
        this.nvt = navigation;
        const {diem,item,itemK,ds,bailam,cauhoi,d0,d1,d2,d3,dapan,socau} = route.params;
        console.log(bailam)
        this.tagsStylesC = {
            span: {
                color:'black',
    ...
    qsExit = () =>
        Alert.alert(
            ...
            [
                ...
                {
                    text: "Có",
                    onPress: ()=>{this.navigation.navigate('huongdan',{id:this.i,ten:this.detai})},
                    style: "cancel",
                },
            ],
            {
    ...
    hienthi(itemQ){
            if(this.state.itemQ == 2){
                ...
                this.setState({
                    ...
                    itemQ:1,
                    q:this.state.qt[0],
                    o:[this.state.opt0[0],this.state.opt1[0],this.state.opt2[0],this.state.opt3[0]],
                    //answ:[...this.state.answ[inum-1]=this.state.trueAns[inum-1]],
                    //opt0:[...this.state.opt0,`${doc.data().Option_ans[0]}`],
                  })
            }
  ...
  render(){
    ...
    const { route,navigation } = this.props;
    this.nvt = navigation;
    const {diem,ds,bailam,cauhoi,d0,d1,d2,d3,dapan,socau,uid,email} = route.params;
    const i = 1;
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.vw1}>
            ...
            <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
                <TouchableOpacity style={{marginLeft:10,flex:1,justifyContent:'flex-start',flexDirection:'row'}} onPress={()=>navigation.navigate('home',{uid:uid,email:email})}>
                    <Icon name = {'arrow-back-ios'} size={20} color={'black'} style={{margin:5,}} />
                </TouchableOpacity>
            </View>
            
            <ScrollView  horizontal={true} showsHorizontalScrollIndicator={false} style={{flexDirection:'column'}}>
                <View style={{}}>
                    <View style={{flexDirection:'row',height:30,}}>
                        {
                            [...Array(socau)].map((o,i) => {
                                if(i+1==l){
                                    return(
                                        <TouchableOpacity key={i} onPress={()=>this.goto(i+1)} style={{
                                                                                            marginLeft:5,
                                                                                            height:35,
                                                                                            width: 50,
                                                                                            alignItems:'center',
                                                                                            justifyContent:'center',}}>
                                                                                                <Icon name = {'arrow-drop-down'} size={30} color={'#1CC625'} style={{}} />
                                        </TouchableOpacity>
                                    )
                        ...
                        [...Array(socau)].map((o,n) => {
                            ...
                            if(bailam[n]!=0){
                                if(bailam[n]==this.state.da[this.kq[this.dem]]){
                                    this.dem++;
                                    return(
                                        <TouchableOpacity key={n} onPress={()=>this.goto(n+1)} style={{borderWidth:2,
                                                                                            margin:5,
                                                                                            height:50,
                                                                                            width: 50,
                                                                                            borderRadius:30,
                                ...
                                return(
                                    <TouchableOpacity key={n} onPress={()=>this.goto(n+1)} style={{borderWidth:2,
                                                                                        ...
                                                                                        alignItems:'center',
                                                                                        justifyContent:'center',}}>
                                                                                        <Text style={{color:this.state.colorBtn}}>{n+1}</Text>
                                    </TouchableOpacity>
                                )
                            }
                        }
                    ...
                    (option, selected, onSelect, index)=>{
                        ...
                        if(option==bailam[l-1]){
                            ...
                            if(option==this.state.dapan){
                                return (
                                    <TouchableOpacity  key={index} style={{borderWidth:2,
                                                                                            borderColor:'#1CC625',
                                                                                            borderRadius:7,
                                                                                            padding:15,
                                                                                            flexDirection:'row',
        ...
        <View style={styles.vw4}>
                ...
                <TouchableOpacity  onPress={()=>navigation.navigate('home',{uid:uid,email:email})} style={{display:this.state.hideHome,
                                                                                        flexDirection:'row',
                                                                                        height:'58%',
                                                                                        alignItems:'center',
                                                                                        justifyContent:'center',
                    ...
                    <View style={styles.btnNext}>
                        <Icon name = {'home'} size={30} color={'#53ad71'} style={{margin:5,}} />
                    </View>
                </TouchableOpacity>
